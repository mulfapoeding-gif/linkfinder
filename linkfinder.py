...
DEFAULT_SITES = [
    ...
    "yts.mx", "yts.lt", "yts.ag", "yts.bz", "yts.unblocked.lol",  # Added yts.bz
    ...
    "ettv.to", "ettv.unblocked.lol",  # TV torrents
    "rarbgmirror.org", "rarbgmirror.com",  # RARBG mirrors
    "yify-torrent.org", "yify-torrent.unblocked.lol",  # Yify mirrors
...
def build_query(term: str, sites: list | None = None, extensions: list | None = None, extra_keywords: str = DEFAULT_EXTRA_KEYWORDS) -> str:
    parts = [term]
    if extra_keywords:
...
# ----------------------------------------------------------------------
def ddg_search(query: str, max_results: int = DEFAULT_MAX_RESULTS, safesearch: str = DEFAULT_SAFESEARCH, backend: str = "auto") -> list[dict]:
    if not _HAS_DDGS:
        raise RuntimeError("DuckDuckGo search library not available")
    with DDGS() as ddgs:
        raw = ddgs.text(query, max_results=max_results, safesearch=safesearch, backend=backend)
    return list(raw)
...
if _HAS_REQUESTS and _HAS_BS4:
    _EXT_RE = re.compile(r"https?://[^\s'\"<>]+(?:{exts})".format(exts="|".join(map(re.escape, DEFAULT_EXTENSIONS))), re.IGNORECASE)
    
    def extract_download_links(page_url: str, extensions: set) -> set[str]:
        try:
            resp = requests.get(page_url, headers={"User-Agent": rand_ua()}, timeout=12, verify=False)
            resp.raise_for_status()
        except Exception:
            return set()
        found = {m.group(0) for m in _EXT_RE.finditer(resp.text) if is_download_url(m.group(0), extensions)}
...
def score_result(item: dict, site_whitelist: set, ext_set: set) -> int:
    ...
    for kw in ("download", "torrent", "magnet", "mp3", "flac", "wav", "movie", "film", "episode", "season", "track", "album", "1080p", "720p", "4k", "bluray", "webrip"):
        if kw in title:
            score += 1
    return score
...
# ----------------------------------------------------------------------
def _run_engine_search(query: str, max_results: int, safesearch: str, backend: str, verbose: bool) -> list[dict]:
    raw = []
    # Try DuckDuckGo first
    try:
        if verbose:
            print("🔎 DuckDuckGo …")
        duck = ddg_search(query, max_results=max_results, safesearch=safesearch, backend=backend)
        raw.extend(duck)
...
def run_search(term: str, sites: list | None = None, extensions: list | None = None, max_results: int = DEFAULT_MAX_RESULTS, extra_keywords: str = DEFAULT_EXTRA_KEYWORDS, safesearch: str = DEFAULT_SAFESEARCH, backend: str = "auto", scrape: bool = True, threads: int = 8, verbose: bool = False) -> list[dict]:
    if extensions is None:
        extensions = DEFAULT_EXTENSIONS
    ext_set = {e.lower() for e in extensions}
    site_set = {s.lower() for s in sites} if sites else set()

    query = build_query(term, sites=sites, extensions=extensions, extra_keywords=extra_keywords)
    if verbose:
    ...
    if not results and (sites or extra_keywords):
        if verbose:
            print("⚡ No hits – trying a broader query (no site filter, no extra keywords)…")
    ...
    if scrape and _HAS_REQUESTS and _HAS_BS4:
        ...
        with ThreadPoolExecutor(max_workers=threads) as exe:
            future_to_item = {exe.submit(extract_download_links, r["href"], ext_set): r for r in results if r.get("href")}
        ...
        for src_url, urls in extra_links.items():
            for dl in urls:
                pseudo = {"title": f"Direct download from {urlparse(dl).netloc}", "href": dl, "source": src_url}
...
def format_as_csv(results: list[dict]) -> str:
    ...
    for r in results:
        writer.writerow([r.get("title", ""), r.get("href", ""), r.get("source", "")])
...
def parse_cli() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Wide‑open search for software, audio, video, torrent links.")
    parser.add_argument("term", nargs="*", help="Search term. If omitted, interactive mode starts.")
    parser.add_argument("-s", "--sites", default=",".join(DEFAULT_SITES), help="Comma‑separated whitelist of sites.")
    parser.add_argument("-e", "--extensions", default=",".join(DEFAULT_EXTENSIONS), help="Comma‑separated file extensions.")
    parser.add_argument("-n", "--max", type=int, default=DEFAULT_MAX_RESULTS, help="Max results (default 60).")
    parser.add_argument("-c", "--no-scrape", action="store_true", help="Skip page‑scraping (faster).")
    parser.add_argument("-t", "--threads", type=int, default=8, help="Scraping threads (default 8).")
    parser.add_argument("-o", "--output", choices=["text", "json", "csv"], default="text", help="Output format.")
    parser.add_argument("-v", "--verbose", action="store_true", help="Show progress messages.")
    parser.add_argument("--enable-safe-search", action="store_true", help="Turn safe‑search ON (default OFF).")
    parser.add_argument("--extra-keywords", action="store_true", help="Add download/install keywords.")
    parser.add_argument("--no-restrict", dest="no_restrict", action="store_true", help="No site whitelist.")
    parser.add_argument("--backend", choices=["auto", "html", "lite", "bing"], default="auto", help="DuckDuckGo backend.")
    parser.add_argument("--version", action="version", version="linkfinder 1.0")
...
def main() -> None:
    ...
    if not args.term:
        ...
        ext_input = input("🔧 Enter comma‑separated extensions (or press ENTER for defaults): ").strip()
        extensions = [e.strip() for e in ext_input.split(",") if e.strip()] if ext_input else None
    ...
    # Normalize arguments
    sites = None if args.no_restrict else [s.strip() for s in args.sites.split(",") if s.strip()]
    ...
    results = run_search(term, sites=sites, extensions=extensions, max_results=args.max, extra_keywords=extra_kw, safesearch=safesearch, backend=args.backend, scrape=not args.no_scrape, threads=args.threads, verbose=args.verbose)
